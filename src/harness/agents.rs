...
impl Default for HarnessConfig {
    fn default() -> Self {
        Self {
            ...
            // Continuous await_turn sessions need many tool rounds per process life
            // (each wake + idle re-poll is a turn). Not a per-game-action cap.
            max_turns_per_tick: 500,
            seed: Some(42),
...
/// `<workdir>/.grok/sessions/{url-encoded-abs-cwd}/{session_id}/signals.json`
///
/// The agent's grok HOME is redirected to its workdir (see `spawn_grok_tick`), so
/// sessions live under the workdir, not the harness's `~/.grok`.
...
fn write_agent_mcp_config(
    ...
    // Timeout layering (seconds):
    //   AWAIT_SERVER_BUDGET (300)  < socket read for await (360)  < await_turn client
    //   (AWAIT_CLIENT_TIMEOUT)     < tool_timeout_sec fallback for *other* tools.
    // tool_timeout_sec is Grok's per-call fallback when a tool has no override;
    // normal tools still hit the socket 120s read first. Keep it high so a mis-set
    // override cannot cancel await_turn earlier than AWAIT_CLIENT_TIMEOUT.
    let await_client = crate::harness::wake::AWAIT_CLIENT_TIMEOUT_SECS;
    let tool_fallback = await_client.saturating_add(60);
    // Base the agent config on the user's global ~/.grok/config.toml with every
    // [mcp_servers.*] table stripped (drops github + its PAT). Keeping the rest
    // preserves the user's model/channel settings — e.g. an alpha-channel default
    // model like `grok-4-5` only resolves with `[cli] channel` + `[models]`
    // present — while, combined with HOME isolation, the agent loads ONLY the botc
    // server appended below. Empty when the user has no global config (botc-only).
    let mut toml = read_global_grok_config_stripped();
    toml.push_str(&format!(
        r#"# Auto-generated by botc-tui — do not commit. Only this MCP server loads.
[mcp_servers.botc]
command = "{bin}"
args = ["--socket", "{sock}", "--token-file", "{tok}", "--game-id", "{game_id}", "--role", "{role_s}"]
enabled = true
startup_timeout_sec = 60
tool_timeout_sec = {tool_fallback}
tool_timeouts = {{ await_turn = {await_client} }}
"#
    ));
    fs::write(workdir.join(".grok/config.toml"), toml)?;
    // The agent's grok HOME is the workdir (see `spawn_grok_tick`), so the config
    // above is the ONLY one it loads — never the global ~/.grok/config.toml's MCP
    // servers or ~/.cursor/mcp.json (pyre-tracer). grok resolves credentials from
    // <HOME>/.grok/auth.json, so seed the isolated home with a copy of the real one.
    copy_grok_auth(workdir);
    Ok(())
}

/// The user's global `~/.grok/config.toml` with every `[mcp_servers.*]` table
/// (including nested tables like `[mcp_servers.github.headers]`) removed, so
/// agents inherit model/channel/UI settings but none of the global MCP servers or
/// the PAT embedded in them. Empty string when there is no readable global config.
fn read_global_grok_config_stripped() -> String {
    let Some(home) = std::env::var_os("HOME") else {
        return String::new();
    };
    match fs::read_to_string(PathBuf::from(home).join(".grok/config.toml")) {
        Ok(src) => strip_mcp_server_tables(&src),
        Err(_) => String::new(),
    }
}

/// Remove every `[mcp_servers.*]` table (including nested tables such as
/// `[mcp_servers.github.headers]`) from a grok config, keeping all other sections.
fn strip_mcp_server_tables(src: &str) -> String {
    let mut out = String::new();
    let mut skipping = false;
    for line in src.lines() {
        let t = line.trim_start();
        if t.starts_with('[') {
            // Any `[mcp_servers…]` header starts a skip; the next non-mcp header ends it.
...
/// Seed the agent's isolated grok HOME with the user's credential so the headless
/// session can authenticate after HOME is redirected to the workdir. Best-effort:
/// on failure the agent fails to authenticate visibly in its stream. The copy is
/// mode 0600 and lives under the work root, which is removed on quit.
fn copy_grok_auth(workdir: &Path) {
    let Some(home) = std::env::var_os("HOME") else {
        crate::dlog!("AUTH copy skipped: no HOME in harness env");
        return;
    };
    let src = PathBuf::from(home).join(".grok/auth.json");
    let dst = workdir.join(".grok/auth.json");
...
"#;
    ...
    #[test]
    fn signals_path_resolves_under_workdir_not_env_home() {
        // The agent's grok HOME is its workdir, so signals must resolve UNDER the
        // workdir — never the harness's $HOME. Regression guard for MCP isolation.
        let dir = std::env::temp_dir().join(format!("botc-sig-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let p = grok_session_signals_path(&dir, "sess-1").expect("path");
        let abs = dir.canonicalize().unwrap();
        assert!(
            p.starts_with(&abs),
            "signals {p:?} must be under workdir {abs:?}, not $HOME"
        );
        assert!(p.ends_with("sess-1/signals.json"), "{p:?}");
        assert!(grok_session_signals_path(&dir, "").is_none());
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn strip_mcp_servers_drops_all_mcp_tables_and_pat_keeps_settings() {
        let cfg = "[cli]\nchannel = \"alpha\"\n\
                   [models]\ndefault = \"grok-4-5\"\n\
                   [mcp_servers.github]\nurl = \"x\"\n\
                   [mcp_servers.github.headers]\nAuthorization = \"Bearer SECRET_PAT\"\n\
                   [ui]\ntheme = \"dark\"\n";
        let s = strip_mcp_server_tables(cfg);
        // Keeps model/channel/UI settings (so the alpha-channel default resolves)...
        assert!(s.contains("channel = \"alpha\""), "keeps [cli]: {s}");
        assert!(s.contains("default = \"grok-4-5\""), "keeps [models]");
        assert!(
            s.contains("theme = \"dark\""),
            "keeps a section that FOLLOWS the mcp block"
        );
        // ...but drops every mcp_servers table and the embedded PAT.
        assert!(!s.contains("mcp_servers"), "drops [mcp_servers.*]: {s}");
        assert!(!s.contains("SECRET_PAT"), "drops the PAT header: {s}");
    }
}
