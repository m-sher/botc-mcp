...
fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "nominate" => "Player nominate a living seat. Engine house rule: usually records an automatic yes for the nominator (Butler may still need a later vote if master has not yesed)",
        "vote" => "Player cast yes/no on the open nomination (once per seat). Trust the live tally — nominator may or may not already have a yes (e.g. Butler). Second vote on the same nomination is rejected",
